...
The product surfaces automated preliminary review only. It is not legal advice, not an advocate or attorney opinion, and not confirmation that a contract is safe to sign.
...
- A release APK with `API_TRANSPORT=http` and an empty `API_BASE_URL` is an offline-local/internal build only; it must not be used as evidence of backend analysis quality
...
- Backend-assisted production requires explicit upload consent, TLS (`https://` public `API_BASE_URL`), zero-retention defaults, no payload/text logs, documented lifecycle controls, and region/data-location review for RF-law risk
...
- Do not commit generated corpora, raw document collections, ad hoc corpus sweeps, smoke runs, timestamped evaluation reports, base64/raw payload dumps, or one-off exported report files
...
- Release notes must state whether the APK was built backend-assisted or offline-local. Backend-assisted APKs must record the inspected `assets/app.config` transport and base URL.
...
- Before distributing any APK, run the runtime config smoke and inspect the packaged `assets/app.config`; an empty backend URL means the APK is offline-local/internal and does not validate backend quality
...
- `docs/privacy_release_readiness.md`
