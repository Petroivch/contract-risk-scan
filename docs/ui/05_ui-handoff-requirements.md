...
## 1) Базовые договоренности (наши строгие ограничения)
- Мы выпускаем приложение только для `Android` и `iPhone`.
- После установки пользователю не нужно докачивать никакие пакеты, модели или графику.
- Наш приоритет в UX — это принцип `local-first` (максимум работы прямо на устройстве).
- Поддерживаемые языки: `ru`, `en`, `it`, `fr`. По умолчанию и при отсутствии перевода всегда используем `ru`.
- Мы строго следим за размером приложения: он не должен превышать `CONST_RELEASE_TOTAL_BUDGET_MB`.
- Мы придерживаемся правила «никакого хардкода»:
  - Любые тексты берем только из ключей локализации (`i18n keys`).
  - Все цвета и отступы — только из дизайн-токенов (`design tokens`).
  - Размеры и лимиты храним в реестре констант (`constants registry`).

## 2) Источники контракта
Мы рекомендуем фронтенд-команде изучать документацию в следующем порядке:
...
- tap по `RiskCard`, `DisputedClauseCard` или секции в `Summary` открывает отдельное окно (Modal / Bottom Sheet) со всей детальной информацией. Inline-раскрытие (аккордеоны) запрещены для предотвращения визуального шума и бесконечного скролла;
