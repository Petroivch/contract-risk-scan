# Config Registry and Invariants

## 1. Назначение
- Убрать magic numbers и дубли enum/политик из кода.
- Обеспечить единый источник значений через `app_config` + `language_catalog`.
...
## 3. Инварианты
1. `app_config.key` по шаблону `^[a-z0-9._-]+$`.
...
- приводит `NULL/invalid` к default языку.

3. `get_config_string(cfg_key)`:
- получает строковые defaults (locale/timezone) из `app_config`.

## 5. Правила изменения конфигов
1. Все новые лимиты добавлять через `app_config` + миграцию.
2. Любой новый enum-сет — через lookup-table (по аналогии `language_catalog`) или документированный schema-level type.
