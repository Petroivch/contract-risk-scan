...
- Мобильное приложение Android + iOS сейчас запускается с `API_TRANSPORT=http`; app-managed SQLite/file cache в runtime по умолчанию выключены.
- `core-api`, если он используется, держит сырой файл договора в памяти только на время активного анализа и не пишет raw upload на диск по умолчанию.
- Отчет и статус backend-сценария считаются runtime state процесса, а не локальным device-only хранилищем клиента.
