...
class RiskScoringService:
    ...
    def _hybrid_match_rule(
        ...
        for feature in clause_features:
            ...
            if not (has_regex_evidence or payment_sanction_guardrail or semantic_evidence):
                continue
    ...
    def _build_risk_template(self, rule: RiskRuleConfig) -> RiskTemplate:
        ...
        for localized_map in (rule.title,):
        ...
        tokens = self._meaningful_tokens(template_text)
    ...
    def _embed_text(self, text: str) -> dict[str, float]:
        tokens = list(self._meaningful_tokens(text))
        if not tokens:
            return {}
    ...
    def _meaningful_tokens(self, text: str) -> set[str]:
        ...
        for token in self._tokenize(text):
            normalized = self._normalize_semantic_token(token)
            if not normalized or normalized in self._semantic_stopwords:
                continue
